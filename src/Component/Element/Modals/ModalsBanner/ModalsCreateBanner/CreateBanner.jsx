...
const CreateBanner = ({ onClose, onUpdate }) => {
  const { create } = useCreate();
  const { upload } = useUpload();
  const [imageBannerUrl, setImageBannerUrl] = useState(null);
  ...
  const handleCreateBanner = async (e) => {
    ...
    try {
      ...
      if (res.status === 200) {
        setPopupMessage("Banner berhasil dibuat.");
        ...
        setPopupMessage("Gagal membuat banner. Status: " + res.status);
        setPopupType("error");
        setPopupVisible(true);
      }
    } catch (error) {
      setPopupMessage("Gagal membuat banner.");
      setPopupType("error");
      console.error(error);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-6 pt-32 pb-10 bg-black bg-opacity-50 lg:px-20">
      <div className="w-auto h-auto p-10 mt-10 mb-10 bg-white rounded-lg z-60">
        ...
        <form onSubmit={handleCreateBanner} className="w-full">
