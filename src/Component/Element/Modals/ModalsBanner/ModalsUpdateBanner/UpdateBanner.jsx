...
const UpdateBanner = ({ onClose, onUpdate, bannerData }) => {
  const { update } = useUpdate();
  const { upload } = useUpload();
  const [imageBannerUrl, setImageBannerUrl] = useState(bannerData.imageUrl);
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      setImageBannerUrl(res.data.url);
      setPopupMessage("");
    } catch (error) {
  ...
  const handleUpdateBanner = async (e) => {
    ...
    if (!bannerData.id) {
      setPopupMessage("ID banner tidak ditemukan.");
      setPopupType("error");
      setPopupVisible(true);
      return;
    }
    ...
    try {
      const res = await update(`update-banner/${bannerData.id}`, updatedBannerData);
      console.log("API Response", res);

      if (res.status === 200) {
        setPopupMessage("Banner berhasil diperbarui.");
        ...
        setPopupMessage("Gagal memperbarui banner. Status: " + res.status);
        setPopupType("error");
        setPopupVisible(true);
      }
    } catch (error) {
      setPopupMessage("Gagal memperbarui banner.");
      setPopupType("error");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="w-full max-w-lg p-10 mt-20 bg-white rounded-lg shadow-lg">
        ...
        <form onSubmit={handleUpdateBanner} className="w-full">
